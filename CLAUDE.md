...
- Built-in server: every build talks to `https://slice-api.fly.dev` (KvittaSyncBaseURL in project.yml) with the trial key from the gitignored ios/Trial.xcconfig, written by tools/trial-config.sh from ~/.kvitta-trial-key (trial.sh runs it; run it by hand before `xcodegen generate` otherwise — xcodegen fails without the file). A build with the key signs in and syncs on its own at first launch (`Bootstrap.adoptBuiltInServer`); the Jag fields are overrides for a LAN/dev server, and a typed address never carries the shipped key (`ServerEndpoint`). Never put KVITTA_TRIAL_KEY in project.yml settings — target settings beat xcconfig and the key silently comes out empty; that is why the plist holds `$(KVITTA_TRIAL_KEY)` and nothing else defines it.
